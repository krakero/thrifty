...
<row class="w-full items-center gap-3">
    <column class="w-[60] items-center rounded-md py-1 {{ $typeClass[0] }}">
        <text font="semibold" class="text-[10] uppercase {{ $typeClass[1] }}" :max-lines="1">{{ $comparable['type'] }}</text>
    </column>
    <text class="flex-1 text-sm text-theme-on-surface" :max-lines="2">{{ $comparable['title'] }}</text>
    <text font="mono" class="text-sm text-theme-on-surface" :max-lines="1">{{ PriceText::keepTogether($comparable['price']) }}</text>
